...
export class SecretObfuscator {
	...
	#collectRegexMatches(
		...
		for (;;) {
			...
			// Skip any match overlapping a placeholder that arrived in the INPUT
			// (origin tag "I" — generated by a prior obfuscate() call). Re-redacting
			// around it corrupts already-emitted output; matches overlapping a FRESH
			// placeholder ("F", created this call) still cross-match, so a raw secret
			// seen again is redacted even when its token equals a prior one.
			if (
				knownPlaceholderRanges.some(range => start < range.end && end > range.start && origin[range.start] === "I")
			) {
				continue;
			}
			if (mode === "replace") {
