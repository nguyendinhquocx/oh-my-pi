...
export class EditTool implements AgentTool<TInput> {
	...
	/** Highest write tier over every target; read only when every target is a read-tier write. */
	readonly approval = (args: unknown): ToolTier => {
		...
		for (const target of targets) {
			const decision = router.writeTier(target, undefined, this.session);
	...
	async #resolveUrl(url: string, context: ResolveContext): Promise<EditUrlResolution> {
		...
		try {
			const located = await router.locate(url, context, { create: true });
